...
exports.KYCIntegrationService = void 0;
...
class KYCIntegrationService {
     ...
     */
    async initializeVerification(userId, walletAddress) {
        const connection = await this.pool.getConnection();
        try {
            await connection.beginTransaction();
            const verificationId = (0, uuid_1.v4)();
            // Create verification record
            await (0, database_1.query)(`INSERT INTO kyc_verifications 
         (id, user_id, provider, status, created_at, updated_at)
         VALUES (?, ?, ?, 'pending', NOW(), NOW())`, [verificationId, userId, this.provider]);
            let redirectUrl = '';
            let providerId = '';
            // Route to appropriate provider
            switch (this.provider) {
                case 'sumsub':
                    ({ redirectUrl, providerId } = await this.initiateSumsubVerification(userId, verificationId, walletAddress));
                    break;
                case 'blockpass':
                    ({ redirectUrl, providerId } = await this.initiateBlockpassVerification(userId, verificationId));
                    break;
                case 'persona':
                    ({ redirectUrl, providerId } = await this.initiatePersonaVerification(userId, verificationId));
                    break;
                case 'mock':
                default:
                    ({ redirectUrl, providerId } = await this.initiateMockVerification(userId, verificationId));
                    break;
            }
            // Store provider ID if applicable
            if (providerId) {
                await (0, database_1.query)(`UPDATE kyc_verifications SET provider_id = ? WHERE id = ?`, [providerId, verificationId]);
            }
     ...
     */
    async initiateSumsubVerification(userId, verificationId, walletAddress) {
        try {
            ...
            const response = await axios_1.default.post('https://api.sumsub.com/resources/applicants?levelName=basic-kyc', {
            ...
            const tokenResponse = await axios_1.default.post(`https://api.sumsub.com/resources/accessTokens?userId=${sumsubApplicantId}`, {}, {
                headers: {
                    'X-App-Token': apiKey
                }
            });
    ...
    async initiateBlockpassVerification(userId, verificationId) {
        try {
            ...
            await (0, database_1.query)(`UPDATE kyc_verifications SET provider_metadata = ? WHERE id = ?`, [JSON.stringify({ blockpassState: state }), verificationId]);
    ...
    async initiatePersonaVerification(userId, verificationId) {
        try {
            ...
            const response = await axios_1.default.post('https://api.withpersona.com/api/v1/inquiries', {
                data: {
                    type: 'inquiry',
                    attributes: {
                        'reference-id': userId
     ...
     */
    async initiateMockVerification(userId, verificationId) {
        return {
    ...
    async getVerificationStatus(verificationId, userId) {
        const result = await (0, database_1.query)(`SELECT * FROM kyc_verifications WHERE id = ? AND user_id = ?`, [verificationId, userId]);
        if (result.length === 0)
            return null;
     ...
     */
    async submitManualKYC(userId, kycData) {
        const connection = await this.pool.getConnection();
        try {
            await connection.beginTransaction();
            const verificationId = (0, uuid_1.v4)();
         ...
         VALUES (?, ?, 'manual', 'pending', ?, NOW(), NOW())`, [verificationId, userId, JSON.stringify(kycData)]);
            // Create admin notification
            const adminUsers = await (0, database_1.query)(`SELECT id FROM users WHERE role IN ('admin', 'super-admin')`);
    ...
    async handleSumsubWebhook(payload) {
        ...
        try {
            ...
            const verifications = await (0, database_1.query)(`SELECT * FROM kyc_verifications WHERE provider_id = ? AND provider = 'sumsub'`, [applicantId]);
            ...
            await (0, database_1.query)(`UPDATE kyc_verifications SET status = ?, verification_date = NOW(), updated_at = NOW() WHERE id = ?`, [newStatus, verification.id]);
     ...
     */
    async handleBlockpassWebhook(payload) {
        const connection = await this.pool.getConnection();
        try {
            await connection.beginTransaction();
            const { referenceId, status } = payload;
            // Find verification
            const verifications = await (0, database_1.query)(`SELECT * FROM kyc_verifications WHERE provider_id = ? AND provider = 'blockpass'`, [referenceId]);
            if (verifications.length === 0) {
    ...
    async handlePersonaWebhook(payload) {
        ...
        try {
            ...
            const verifications = await (0, database_1.query)(`SELECT * FROM kyc_verifications WHERE provider_id = ? AND provider = 'persona'`, [inquiryId]);
            if (verifications.length === 0) {
    ...
    async getPendingVerifications(limit = 50, offset = 0) {
        return (0, database_1.query)(`SELECT kv.*, u.email, u.username, u.wallet_address
    ...
    async approveVerification(verificationId) {
        ...
        try {
            await connection.beginTransaction();
            const verifications = await (0, database_1.query)(`SELECT * FROM kyc_verifications WHERE id = ?`, [verificationId]);
            if (verifications.length === 0) {
                throw new Error('Verification not found');
            }
            const verification = verifications[0];
            await (0, database_1.query)(`UPDATE kyc_verifications SET status = 'approved', verification_date = NOW(), updated_at = NOW() WHERE id = ?`, [verificationId]);
         ...
         VALUES (?, ?, 'kyc_approved', 'KYC Approved', 'Your KYC verification has been approved!', NOW())`, [notificationId, verification.user_id]);
            await connection.commit();
    ...
    async rejectVerification(verificationId, reason) {
        ...
        try {
            ...
            const verification = verifications[0];
            await (0, database_1.query)(`UPDATE kyc_verifications SET status = 'rejected', rejection_reason = ?, updated_at = NOW() WHERE id = ?`, [reason, verificationId]);
            // Create notification
            const notificationId = (0, uuid_1.v4)();
            await (0, database_1.query)(`INSERT INTO notifications 
         (id, user_id, type, title, message, created_at)
...
exports.KYCIntegrationService = KYCIntegrationService;
//# sourceMappingURL=kycIntegrationService.js.map