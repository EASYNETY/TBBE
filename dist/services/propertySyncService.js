...
exports.propertySyncService = exports.PropertySyncService = void 0;
const ethers_1 = require("ethers");
const database_1 = require("../utils/database");
const tbaService_1 = require("./tbaService");
const marketplaceRegistrationService_1 = require("./marketplaceRegistrationService");
...
class PropertySyncService {
    ...
    async initializeSyncState() {
        try {
            ...
            const result = await (0, database_1.query)('SELECT value FROM system_settings WHERE key = ?', ['last_synced_block']);
    ...
    async syncTitleRegistrarEvents(fromBlock, toBlock) {
    ...
    async syncTBAEvents(fromBlock, toBlock) {
    ...
    async syncMarketplaceEvents(fromBlock, toBlock) {
    ...
    async syncPropertyRegistryEvents(fromBlock, toBlock) {
    ...
    async processTitleRegistrarEvent(event) {
        if (event.eventName === 'PropertyMinted') {
            ...
            await (0, database_1.query)('UPDATE properties SET token_id = ?, status = "MINTED", minted_at = CURRENT_TIMESTAMP WHERE metadata_hash = ? AND (token_id IS NULL OR token_id = ?)', [Number(tokenId), metadataURI.replace('https://gateway.pinata.cloud/ipfs/', 'ipfs://'), Number(tokenId)]);
            // Trigger next steps in the flow
            await this.triggerPostMintSteps(Number(tokenId));
        }
    }
    async processTBAEvent(event) {
        if (event.eventName === 'AccountCreated') {
            ...
            await (0, database_1.query)('UPDATE properties SET tba_address = ? WHERE token_id = ? AND (tba_address IS NULL OR tba_address = ?)', [account, Number(tokenId), account]);
        }
    }
    async processMarketplaceEvent(event) {
        if (event.eventName === 'TokenRegistered') {
            ...
            await (0, database_1.query)('UPDATE properties SET marketplace_registered = TRUE WHERE token_id = ? AND marketplace_registered = FALSE', [Number(tokenId)]);
        }
    }
    async processPropertyRegistryEvent(event) {
        if (event.eventName === 'PropertyApproved') {
            ...
            await (0, database_1.query)('UPDATE properties SET status = "APPROVED", approved_at = CURRENT_TIMESTAMP WHERE property_registry_id = ? AND status = "PENDING"', [Number(propertyId)]);
            // Trigger minting process
            await this.triggerMintingProcess(Number(propertyId));
        }
    }
    async triggerMintingProcess(propertyRegistryId) {
        try {
            // Find the property by registry ID
            const property = await (0, database_1.query)('SELECT id FROM properties WHERE property_registry_id = ? AND status = "APPROVED"', [propertyRegistryId]);
    ...
    async triggerPostMintSteps(tokenId) {
        try {
            // Get property details
            const property = await (0, database_1.query)('SELECT id, project_id FROM properties WHERE token_id = ?', [tokenId]);
            if (!property || property.length === 0)
                return;
            const propertyId = property[0].id;
            ...
            const user = await (0, database_1.query)('SELECT wallet_address FROM users WHERE id = (SELECT user_id FROM properties WHERE id = ?)', [propertyId]);
            if (user && user.length > 0) {
                await tbaService_1.tbaService.createTBA(tokenId, user[0].wallet_address);
            }
            // Step 2: Register with marketplace
            console.log(`Registering token ${tokenId} with marketplace`);
            if (projectId) {
                await marketplaceRegistrationService_1.marketplaceRegistrationService.setupMarketplaceReadiness(tokenId, projectId);
            }
            ...
            await (0, database_1.query)('UPDATE properties SET status = "ACTIVE", activated_at = CURRENT_TIMESTAMP, trade_ready = TRUE WHERE id = ? AND status = "MINTED"', [propertyId]);
            // Step 5: Notify user
    ...
    async emitUserNotification(propertyId, type, message) {
        try {
            // Get user ID from property
            const property = await (0, database_1.query)('SELECT user_id FROM properties WHERE id = ?', [propertyId]);
            if (!property || property.length === 0)
                return;
            const userId = property[0].user_id;
            // Insert notification (assuming notifications table exists)
            await (0, database_1.query)('INSERT INTO notifications (user_id, type, message, created_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)', [userId, type, message]);
    ...
    async updateLastSyncedBlock(blockNumber) {
        await (0, database_1.query)('INSERT INTO system_settings (`key`, value) VALUES (?, ?) ON DUPLICATE KEY UPDATE value = ?', ['last_synced_block', blockNumber.toString(), blockNumber.toString()]);
    }
    // Manual sync trigger for specific property
    async syncPropertyStatus(propertyId) {
        try {
...
exports.PropertySyncService = PropertySyncService;
exports.propertySyncService = new PropertySyncService();
//# sourceMappingURL=propertySyncService.js.map