...
async function seedData() {
    try {
        ...
        const existingUser = await (0, database_1.query)('SELECT id FROM users WHERE id = ?', [userId]);
        if (existingUser.length === 0) {
            await (0, database_1.query)('INSERT INTO users (id, wallet_address, username, role, permissions, is_active, email) VALUES (?, ?, ?, ?, ?, ?, ?)', [userId, walletAddress, 'Platform Admin', 'admin', JSON.stringify([]), true, 'admin@titlebase.com']);
        ...
        for (const prop of buyProperties) {
            const existing = await (0, database_1.query)('SELECT id FROM properties WHERE id = ?', [prop.id]);
            if (existing.length === 0) {
                await (0, database_1.query)('INSERT INTO properties (id, owner_id, title, description, address, property_type, square_footage, bedrooms, bathrooms, year_built, lot_size, images, assessed_value, verification_status) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, 0, 0, ?, ?, ?)', [prop.id, userId, prop.title, `Premium ${prop.title}`, prop.address, 'residential', prop.images, prop.assessed_value, 'verified']);
                const listingId = (0, uuid_1.v4)();
                await (0, database_1.query)('INSERT INTO listings (id, property_id, seller_id, listing_type, price, currency, start_time, status) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?)', [listingId, prop.id, userId, 'fixed_price', prop.price, 'ETH', 'active']);
        ...
        for (const prop of bidProperties) {
            ...
            if (existing.length === 0) {
                ...
                await (0, database_1.query)('INSERT INTO listings (id, property_id, seller_id, listing_type, price, currency, start_time, end_time, status) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, ?)', [listingId, prop.id, userId, 'auction', prop.price, 'ETH', endTime, 'active']);
