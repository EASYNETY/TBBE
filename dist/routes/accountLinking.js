...
exports.createAccountLinkingRoutes = void 0;
const express_1 = __importDefault(require("express"));
const accountLinkingService_1 = require("../services/accountLinkingService");
const auth_1 = require("../middleware/auth");
const createAccountLinkingRoutes = (db) => {
    const router = express_1.default.Router();
    const linkingService = new accountLinkingService_1.AccountLinkingService(db);
     ...
     */
    router.get('/primary', auth_1.authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
     ...
     * Requires: linkId
     */
    router.post('/set-primary', auth_1.authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
     ...
     * Requires: linkId
     */
    router.post('/unlink', auth_1.authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
     ...
     * Requires: linkId
     */
    router.post('/resend-code', auth_1.authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
     ...
     */
    router.get('/audit/:linkId', auth_1.authenticateToken, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
...
exports.createAccountLinkingRoutes = createAccountLinkingRoutes;
//# sourceMappingURL=accountLinking.js.map