...
import { KYCIntegrationService } from '../services/kycIntegrationService';
import { authenticateToken, AuthRequest } from '../middleware/auth';
import crypto from 'crypto';

export const createKYCIntegrationRoutes = (db: Pool) => {
  const router = express.Router();
  const kycService = new KYCIntegrationService(db);
   ...
   */
  router.get('/status/:verificationId', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
   ...
   */
  router.post('/admin/approve/:verificationId', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user || !['admin', 'super-admin'].includes(req.user.role)) {
        return res.status(403).json({ error: 'Admin access required' });
      }
  ...
  /**
    ...
    */
   router.post('/admin/reject/:verificationId', authenticateToken, async (req: AuthRequest, res) => {
     try {
       if (!req.user || !['admin', 'super-admin'].includes(req.user.role)) {
         return res.status(403).json({ error: 'Admin access required' });
       }
   ...
   router.post('/mock/complete/:verificationId', async (req, res) => {
     try {
       ...
       const verifications = await query(
         `SELECT * FROM kyc_verifications WHERE id = ? AND provider = 'mock'`,
         [verificationId]
       );
       ...
       // Update verification
       await query(
         `UPDATE kyc_verifications SET status = ?, verification_date = NOW(), kyc_data = ?, updated_at = NOW() WHERE id = ?`,
         [status, kycData ? JSON.stringify(kycData) : null, verificationId]
       );

       // If approved, mark wallet as KYC verified
       if (approved) {
         await query(
           `UPDATE wallets SET kyc_status = 'verified', kyc_verified_at = NOW() WHERE user_id = ?`,
