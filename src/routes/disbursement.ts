...
export const createDisbursementRoutes = (db: Pool) => {
   ...
   */
  router.get('/subscription/:subscriptionId', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
   ...
   */
  router.get('/:disbursementId', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
   ...
   */
  router.get('/schedule/:subscriptionId', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
  ...
  router.post('/schedule', authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const schedule = await disbursementService.createDisbursementSchedule(
        subscriptionId,
        req.user.id,
        amount,
        roiPercentage,
   ...
   */
  router.post('/:disbursementId/process', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
   ...
   */
  router.post('/:disbursementId/retry', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
   ...
   */
  router.post('/schedule/:subscriptionId/pause', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
   ...
   */
  router.post('/schedule/:subscriptionId/resume', authenticateToken, async (req: AuthRequest, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized' });
      }
