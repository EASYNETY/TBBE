...
import { SubscriptionDistributionChannelService } from '../services/subscriptionDistributionChannelService';
...
async function main() {
  try {
    ...
    const channelService = new SubscriptionDistributionChannelService(db);
    ...
    const executionResults = await subscriptionService.executeBatchDisbursements(
      disbursementIds,
      tokenAddress
    );
    ...
    Object.entries(report.subscription_breakdown.by_currency).forEach(([currency, data]: any) => {
      console.log(`    ${currency}: ${data.count} subscribers, ${data.total_amount} total`);
    });

    console.log('\n  Disbursements by Type:');
    Object.entries(report.disbursement_breakdown.by_type).forEach(([type, data]: any) => {
      console.log(`    ${type}: ${data.count} disbursements, ${data.total_amount} total`);
    });

    console.log('\n  Disbursements by Status:');
    Object.entries(report.disbursement_breakdown.by_status).forEach(([status, count]: any) => {
    ...
    const historyDisbursements = await subscriptionService.getDisbursementsByDateRange(
      propertyId,
      startDate,
      endDate
    );
    ...
    console.log(
      `Found ${historyDisbursements.length} disbursements in last 30 days:`
    );
    ...
    // Verify cancellation
    const verifyCancel = await subscriptionService.getSubscriptionsBySubscriber(
      subscriptionToCancel.subscriber_user_id
    );
